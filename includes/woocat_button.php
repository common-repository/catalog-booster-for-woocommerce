...
function ic_woocat_enable_button() {
	...
	if ( ! empty( $ic_woocat['button']['enable'] ) && is_product() ) {
		add_action( 'woocommerce_single_product_summary', 'ic_woocat_show_button' );
		add_action( 'product_details', 'ic_woocat_show_button' );
	}
}
...
function ic_woocat_show_button() {
	...
	if ( ! empty( $ic_woocat['button']['enable'] ) ) {
		...
		if ( ! empty( $url ) ) {
			?>
            <div class="ic-button-container"><a href="<?php echo $url ?>"
                                                class="button"><?php echo $ic_woocat['button']['label'] ?></a></div>
			<?php
		}
	}
}
...
if ( ! function_exists( 'ic_woocat_price' ) ) {
	...
	function ic_woocat_price( $price = null, $product_id = null ) {
		global $product;
		if ( ( empty( $product ) || ! method_exists( $product, 'get_price_html' ) ) && ! empty( $product_id ) ) {
